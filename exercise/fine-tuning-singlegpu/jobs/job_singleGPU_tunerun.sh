#!/bin/bash -e
#SBATCH --job-name=1FineTune_llama3_11B_1GPU_tunerun
...
# QA
CONFIG_FILE="${CONFIG_DIR}/11B_lora_single_device.yaml"

PYTHON_FILE="${PYTHON_DIR}/lora_finetune_single_device.py"

# Define the output & logging directories for fine-tuning results
OUTPUT_DIR="$MyWD/data/Llama-3.2-11B-Vision-Instruct_out_tune"
LOGGING_DIR="$MyWD/data/lora_finetune_11B_output_tune"
...
tune run lora_finetune_single_device --config "${CONFIG_FILE}" checkpointer.output_dir="${OUTPUT_DIR}" output_dir="${LOGGING_DIR}" epochs=1
...
time apptainer exec --nv -B "${MyWD}:${MyWD}" \
      "${APPTAINER_SIF}" \
      "${INNER_SCRIPT_TEMP}"
