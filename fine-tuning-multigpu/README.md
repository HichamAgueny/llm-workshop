...
1. **Explore the SLURM job script**  
   - Navigate to the `jobs/` directory:
     ```bash
     cd /cluster/work/projects/nn9970k/$USER/llm-workshop/fine-tuning-multigpu/jobs
     ````
...
2. **Check the configuration file**  
   - Go to the `config/` directory:
     ```bash
      cd /cluster/work/projects/nn9970k/$USER/llm-workshop/fine-tuning-multigpu/config_scripts
     ````
...
* The slurm output file is saved in:
  ...
  ```
  /cluster/work/projects/nn9970k/$USER/llm-workshop/fine-tuning-multigpu/jobs/out
  ```

4. **Monitoring GPU Usage**
A script for monitoring the GPU usage and GPU memory utilization is made available in:
```
/cluster/work/projects/nn9970k/$USER/llm-workshop/tools/bin
```
And this path ```export PATH="/cluster/work/projects/nn9970k/$USER/llm-workshop/tools/bin:$PATH"``` is already added to your ´.bashrc`. You simply need to source it:
```bash
source ~/.bashrc
...
```bash
   cd /cluster/work/projects/nn9970k/$USER/llm-workshop/data/lora_finetune_QA_output_4GPU
   ls
   ````
...
```bash
   /cluster/work/projects/nn9970k/$USER/llm-workshop/data/Llama-3.2-1B-Instruct_QA_out_4GPU_onlyLoRAweight
````
