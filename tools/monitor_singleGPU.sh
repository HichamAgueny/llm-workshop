...
srun --jobid=$JOB_ID --interactive --pty bash  #<<'INNER_EOF'
echo "Inside job environment..."

# Get GPU UUID(s) associated with compute processes
echo "Querying nvidia-smi for PID and GPU UUID..."
output=\$(nvidia-smi --query-compute-apps=pid,gpu_uuid --format=csv,noheader,nounits)

# Read first line of output into two variables: PID and GPU_UUID
IFS=',' read -r PID GPU_UUID <<< "\$output"
...
watch -n 1 "nvidia-smi --id=\$GPU_UUID --query-gpu=utilization.gpu,memory.used,temperature.gpu,power.draw --format=csv,noheader,nounits | awk -f $TOOLS_DIR/gpu_format.awk"
#INNER_EOF
